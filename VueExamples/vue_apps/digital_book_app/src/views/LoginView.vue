  ...
  <div class="login-outer-container">
    <div class="login-inner-container">
      ...
      <div class="login-right">
        ...
        <div class="form-options" :class="{ 'single-item': loginInfo.isSmsLogin }">
          <div v-show="!loginInfo.isSmsLogin" class="remember-me">
            <input id="remember-me" v-model="loginInfo.rememberMe" type="checkbox" />
            <label for="remember-me">记住密码</label>
          </div>
          <div class="switch-login" @click="switchLogin">{{ loginInfo.switchText }}</div>
        </div>
        ...
        <div class="agreement">
          ...
          <label for="agreement"
            >阅读并同意 <a href="https://www.unischool.cn/student-pc/user">《用户服务协议》</a> 和
            <a href="https://www.unischool.cn/student-pc/privacy">《隐私保护政策》</a></label
          >
        </div>
      </div>
    </div>
...
<script setup>
  ...
  const handleLogin = debounce(async () => {
    ...
    const { access_token: accessToken } = await getTokenInfo(
      loginInfo.username,
      loginInfo.password,
      openid,
      serviceTicket
  ...
  const switchLogin = () => {
    ...
    loginInfo.switchText = loginInfo.isSmsLogin ? "账号密码登录" : "验证码登录"
  }
  ...
  const canLogin = computed(() => {
    if (loginInfo.isSmsLogin) {
      return loginInfo.phoneNumber && loginInfo.captcha && loginInfo.agreementAccepted
    } else {
